...
function PoissonProcess(t, λ)
	...
	while true
		time = arrival_times[end] + rand(Exponential(1/λ))
		if time > t
			break
		else
			append!(arrival_times, time)
...
function PoissonProcess2(t, λ)
	# Number of events over time period follows Poisson dsitribution
	num_events = rand(Poisson(t * λ))
	# Cumulative number of events
	process = vcat(range(0, num_events), num_events)
	# Event times are uniformly distributed over the time interval, then sorted
	# Including process starting at 0 and ending at t
	event_times = vcat(0, sort(rand(Uniform(0, t), num_events)), t)
	return event_times, process
end
...
# Algorithm 6
function InhomogeneousPoissonProcess(t, λₜ::Function, λᵤ)
	# Process starts at o
	time = 0
	process = [0]
	arrival_times = [0.0]
	while true
		time = time + rand(Exponential(1/λᵤ))
